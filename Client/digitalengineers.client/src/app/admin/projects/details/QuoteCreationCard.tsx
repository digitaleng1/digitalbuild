...
const QuoteCreationCard = ({ projectId, project, onQuoteSubmitted }: QuoteCreationCardProps) => {
	...
	const [quotedAmount, setQuotedAmount] = useState<string>('');
		...
		const isQuotePending = project.status === 'QuotePending';
		const isQuoteSubmitted = project.status === 'QuoteSubmitted';
		const isQuoteRejected = project.status === 'QuoteRejected';
		const isQuoteAccepted = project.status === 'QuoteAccepted';
